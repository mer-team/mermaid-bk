...
async function saveLog(msg){
    await Log.create({
        ...
        song_id: getSongId(msg), 
        type: "info"
    }).then(log => {
        return res.status(200).json("Log created")
    }).catch(e => {
        console.log(e)
    })
}
      ...
      search(`https://www.youtube.com/watch?v=${id}`, opts, async function(err, results) {
        if(err) return console.log(err);
            ...
            await Song.create({
                ...
                thumbnailHQ: results[0].thumbnail.high.url, 
                thumbnailMQ: results[0].thumbnail.medium.url, 
                hits: 0,
...
async function classifySong(msg){
